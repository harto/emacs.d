...
;; Put temp and config files in a consistent place. (This should be configured
;; early, before packages start putting their files everywhere).
(use-package no-littering
  :demand t
  :custom
  ;; Avoid collocating backup and auto-save files with their originals, so as
  ;; not to interfere with filesystem watchers (auto-recompilation processes,
  ;; etc.)
...
;; Write autogenerated Customize config to a throwaway(ish) file, not here.
;; Most custom vars are set directly via use-package.
...
;; Trim extraneous whitespace (modified lines only) on save
(use-package ws-trim
  :demand t
  :config
...
;;
;; These prefixes are roughly used as follows:
...
;; have to use the mouse to close it. Instead, just unbind s-t.
...
(use-package newcomment
  :custom
  ;; Also comment out empty lines in selected region.
...
(use-package isearch
  :bind (:map isearch-mode-map
              ("M-." . 'sc/isearch-yank-symbol-at-point)))
...
(use-package ido
  ...
  (ido-decorations (list "\n-> " ""     ; "brackets" around prospect list
                         ...
                         " [Too big]"
                         " [Confirm]"))
  ;; There's no way to completely disable ido's auto-merge behaviour, but we can
  ;; delay it for a really long time.
...
;; TODO: find & replace in directory tree
;; Maybe based on project-query-replace-regexp?
;; See also:
;; - https://www.reddit.com/r/emacs/comments/1ga944z/how_do_you_findreplace_in_a_project_or_current/
...
With \\[universal-argument], submits a draft PR.

With \\[universal-argument] \\[universal-argument], prompts for base branch and whether to submit a draft PR."
    (interactive "p")
    (let ((opts (list "--browse")))
      ...
      (cond ((equal configure 4)
            ...
            ((equal configure 16)
             (when-let ((base (magit-read-local-branch "Base branch" (magit-main-branch))))
...
;; git-commit is a dependency of magit, used to edit commit messages and PR
;; descriptions. We don't need to use it directly except to configure the max
;; line length for PR descriptions. (There might be an easier way to do this...)
  ...
  (defun sc/set-git-commit-line-limits ()
    ...
    (if (equal (buffer-name) "PULLREQ_EDITMSG")
        (progn
          ...
          ;; Use `most-positive-fixnum', rather than nil, so that fill-paragraph
          ;; & fill-region continue to work as expected.
...
(use-package compile
  ...
  :config
  ;; An attempt to handle persistent processes (e.g. tsc --watch) via compile.
...
(use-package lsp-mode
  ...
  ;; Don't want header line
  (lsp-headerline-breadcrumb-enable nil)
  ;; Don't currently use yasnippet
  (lsp-enable-snippet nil))

;; Company is automatically required by lsp-mode.
;; Listing it here so I don't accidentally uninstall it.
;; (use-package company)
...
;; Tree-sitter language grammars. We only need to uncomment/use this when we're
;; installing a new grammar.
;;
...
(use-package sql
  ...
  (defun sc/pop-to-sql-query-buffer ()
    "Jump to a dedicated query buffer for the current SQLi connection."
    (interactive)
...
;; TODO: more fully partition this into personal & work stuff
