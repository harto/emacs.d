...
;; Configuration set via the Customize system is automatically put in this file by default. I don't
;; want a bunch of autogenerated cruft added here, so I store it in a separate file. Also, the
;; use-package macro has a way to specify Custom variables inline, so I prefer to do that where
;; possible. (TODO: move remaining config from custom file to this file)
(setq custom-file "~/.emacs-custom.el")
(load custom-file nil t)

;; Put temp and config files in a consistent place. This package should be configured early (before
;; packages start putting their files everywhere).
(use-package no-littering
  :custom
  ;; Put Emacs backup and auto-save files in a centralised (out-of-the-way) location. By default
  ;; Emacs stores auto-save files (etc.) alongside the original files, which interferes with tools
  ;; that watch directories for changes (auto-recompilation processes, Dropbox, etc.)
...
;; # Console setup
;;
;; (Commented-out because I haven't used this in years, and it likely doesn't work anymore.)
...
;; line-number-mode is disabled when changing themes for some reason, so make sure to reenable it
;; when we do that.
(add-hook 'sc/load-theme-hooks (lambda (_theme) (line-number-mode)))

;; Solarized adds top & bottom borders the modeline, but we use a custom modeline and the borders
;; look bad there.
...
;; Treat components of CamelCasedWords as individual words when moving the cursor forwards &
;; backwards by word. i.e. with point like "|FooBar", pressing `M-f` moves to "Foo|Bar", not
;; "FooBar|".
(use-package subword
  :hook ((js2-mode python-mode ruby-mode typescript-mode) . subword-mode))

;; Automatically insert closing parentheses, brackets, etc. in certain (non-Lisp) modes. (In Lisp
;; code we use paredit instead.)
...
(use-package ido
  ...
  (ido-decorations (list "\n-> " ""     ; "brackets" around prospect list
                         ...
                         " [Too big]"
                         " [Confirm]"))
  ;; I prefer not to use ido's auto-merge behaviour, so I "disable" it by delaying it for a really
  ;; long time.
  ...
  (ido-mode +1)
  (ido-everywhere +1)
  ;; flx-ido (https://github.com/lewang/flx) replaces the ido sorting algorithm for better
  ;; fuzzy-matching.
  (flx-ido-mode +1))

;; find-things-fast helps with locating files in git repos (and other kinds of "projects").
(use-package find-things-fast
  ...
  :config
  ;; When searching and grepping in a non-git context, i.e. using `find`, make sure to only include
  ;; regular files (not directories, symlinks, etc.)
  (advice-add 'ftf-get-find-command
              :filter-return
              (lambda (find-command)
                (concat find-command " -type f")))

  ;; Provide a way to exclude files from searches. find-things-fast already excludes git-ignored
  ;; files, but we might also want to exclude certain versioned files (fixtures, package lockfiles,
  ;; etc.).
...
;; git-commit (a dependency of magit) is used when editing commit messages and PR descriptions. I
;; configure this because I want to unset the max line length for pr descriptions. (There might be
...
(use-package flycheck
  ...
  :config
  ;; TODO: instead of this, rebind default Flycheck prefix (`C-c !`) to something more usable
...
(use-package elisp-mode
  ...
  ;; Sometimes I type non-Lisp stuff into the *scratch* buffer and I don't want strict parenthesis
  ;; matching.
...
(use-package python
  :defer t
  :custom
  ;; Disable pydoc pager so that `help(…)` doesn't complain about not having a fully-functional
  ;; terminal. Note: the final print() is required, because `python-shell-setup-code' seems to block
  ;; until some kind of output is sent.
...
(load "utils")
;; My library of work helper functions lives outside this repo, so I don't accidentally reveal
;; anything sensitive.
